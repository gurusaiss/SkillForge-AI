...
class SmartAgent {
  ...
  async processGoal(goalText, existingUserId, profilingData = null) {
    ...
    const diagnosticQuestions = await this.quizGenerator.generateKnowledgeQuiz(skillTree);
    ...
    const agentDecisions = [
      {
        ...
        reasoning: `${aiPowered ? 'AI analyzed' : 'Keyword scan of'} goal text. Domain "${skillTree.domain}" matched with highest confidence. Detected tools: ${skillTree.profile.detectedTools.join(', ') || 'none specified'}.`,
      },
      {
        id: 2,
        timestamp: new Date().toISOString(),
        ...
        reasoning: `${aiPowered ? 'AI generated domain-specific skill tree.' : 'Skills ranked by relevance to goal keywords and detected tools.'} Days allocated proportionally — more for foundational skills.`,
      },
      {
        id: 3,
        timestamp: new Date().toISOString(),
        ...
        reasoning: `${aiPowered ? 'AI created domain-specific questions — real concepts, not generic.' : 'Questions selected by relevance score to goal keywords.'} Open-ended questions capture reasoning depth.`,
      },
    ];
  ...
  async submitSession({ userId, day, skillId, challenge, userResponse }) {
    ...
    session.agentDecisions.push({
      ...
      reasoning: `Evaluated against ${challenge.evaluation_criteria?.length || 0} criteria. ${evaluation.source === 'llm' ? 'AI provided nuanced assessment.' : 'Keyword matching + response depth scoring.'}`,
    });
