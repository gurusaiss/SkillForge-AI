  ...
  scoreDiagnostic(questions, answers) {
    ...
    for (const question of questions) {
      ...
      if (question.type === 'multiple_choice') {
        ...
        const acceptable = (question.acceptable_answers || [question.correct]).map(a => a.trim().toLowerCase());
        const isCorrect = acceptable.some(a => answerText === a || answerText.includes(a) || a.includes(answerText));
        score = isCorrect ? 100 : answerText.length > 0 ? 20 : 0;
        feedback = isCorrect
          ? `Correct! "${question.correct}" is the right answer. ${question.explanation}`
          : `The expected answer was "${question.correct}". ${question.explanation}`;

      } else if (question.type === 'subjective' || question.type === 'open_ended') {
        ...
        const lengthBonus = wordCount > 80 ? 25 : wordCount > 50 ? 20 : wordCount > 25 ? 10 : wordCount > 10 ? 5 : 0;
        score = Math.min(baseScore + lengthBonus, 95);

        if (score >= 70) {
          feedback = `Strong answer. You covered ${matchedKeywords.slice(0, 4).join(', ')}. ${question.explanation}`;
        } else if (score >= 40) {
