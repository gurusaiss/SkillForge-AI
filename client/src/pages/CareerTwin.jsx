...
export default function CareerTwin() {
  ...
  const radarData = forecast?.opportunityRadar || skills.slice(0, 6).map(s => {
    const target = s.targetMastery || (s.priority === 'high' ? 90 : s.priority === 'medium' ? 80 : 75);
    return { skill: s.name, current: s.mastery || 0, target, marketDemand: s.marketRelevance || 75, gap: target - (s.mastery || 0) };
  });
