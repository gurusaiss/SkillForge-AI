...
export default function Diagnostic() {
  ...
  // Knowledge Quiz: 5 MCQ only
  const allQuestions = (initialData?.diagnosticQuestions || [])
    .filter(q => !q.type || q.type === 'multiple_choice')
    .slice(0, 5);
  ...
  const selectOption = (option) => {
    ...
    if (!isLast && currentQuestion?.type === 'multiple_choice') {
      setTimeout(() => {
        setAnimating(true);
        setTimeout(() => {
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            {currentQuestion.type === 'fill_in_blank' && (
              <span className="text-[9px] px-2.5 py-1 rounded-full bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 font-bold">
                ✏️ Fill in Blank
              </span>
            )}
            {currentQuestion.type === 'subjective' && (
              <span className="text-[9px] px-2.5 py-1 rounded-full bg-purple-500/15 text-purple-300 border border-purple-500/30 font-bold">
                📝 Subjective
              </span>
            )}
          ...
          {currentQuestion.type === 'fill_in_blank' && (
            <div className="space-y-3">
              <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 px-4 py-3 text-xs text-indigo-300">
                ✏️ Type the word or phrase that best fills in the blank.
              </div>
              <input
                type="text"
                value={currentAnswer || ''}
                ...
                className="w-full rounded-xl border border-slate-600 bg-slate-800 px-4 py-3 text-sm text-slate-100 placeholder-slate-600 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
              />
              {currentAnswer && (
                <div className="flex items-center gap-1.5 text-xs text-emerald-400">
                  <span>✓</span>
                  <span>Answer recorded — click Next or change your answer above</span>
                </div>
              )}
            </div>
          )}
          ...
          {currentQuestion.type === 'subjective' && (
            <div className="space-y-3">
              <div className="rounded-xl border border-purple-500/30 bg-purple-500/5 px-4 py-3 text-xs text-purple-300">
                📝 Write a detailed answer. Aim for 3–6 sentences. Your response will be evaluated by AI.
              </div>
              {currentQuestion.sample_good_answer && (
                <div className="rounded-xl border border-slate-700/40 bg-slate-800/30 px-4 py-3">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">What a good answer covers</p>
                  <p className="text-xs text-slate-400 leading-relaxed">{currentQuestion.sample_good_answer}</p>
                </div>
              )}
              <textarea
                value={currentAnswer || ''}
                ...
                rows={5}
                className="w-full rounded-xl border border-slate-600 bg-slate-800 px-4 py-3 text-sm text-slate-100 placeholder-slate-600 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500 transition-all resize-none"
              />
              <div className="flex items-center justify-between text-xs">
                <span className={`${(currentAnswer || '').length < 50 ? 'text-slate-600' : 'text-emerald-400'}`}>
                  {(currentAnswer || '').length} characters {(currentAnswer || '').length < 50 ? '(aim for 50+)' : '✓'}
                </span>
                {(currentAnswer || '').split(/\s+/).filter(Boolean).length > 0 && (
                  <span className="text-slate-600">{(currentAnswer || '').split(/\s+/).filter(Boolean).length} words</span>
                )}
              </div>
            </div>
          )}
      ...
      {allAnswered && (
        <div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-center">
          <p className="text-xs text-emerald-400 font-bold">
            ✅ All {questions.length} questions answered (7 MCQ · 2 Fill-in-Blank · 1 Subjective) — ready to generate your personalized plan!
          </p>
        </div>
      )}
    </div>
