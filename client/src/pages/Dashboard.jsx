  ...
  return (
    <div className={`w-full bg-slate-800 rounded-full overflow-hidden ${thin ? 'h-1.5' : 'h-2'}`}>
      <div
        ...
        style={{ width: value > 0 ? `${Math.max(value, 2)}%` : '0%', backgroundColor: color }}
      />
    </div>
  );
}
            ...
            <div className="space-y-5">
              ...
              <div className="border-t border-slate-800 pt-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Confidence Calibration</h3>
                  <span className="text-[9px] text-slate-600 bg-slate-800/60 px-2 py-0.5 rounded-full border border-slate-700/50">
                    Rate confidence before sessions to unlock
                  </span>
                </div>
                <ConfidenceHeatmap sessions={sessions} />
              </div>
              {/* Learner profile */}
              <div className="border-t border-slate-800 pt-4">
